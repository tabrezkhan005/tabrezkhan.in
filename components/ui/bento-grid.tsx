...
const BentoCard = ({
  ...
  >
    ...
    {/* Bottom content area */}
    <div className="relative z-10 p-6 pt-10 bg-gradient-to-t from-black/90 via-black/60 to-transparent">
      <div className="pointer-events-none flex transform-gpu flex-col gap-1 transition-all duration-300 group-hover:-translate-y-10">
        {Icon && <Icon className="h-8 w-8 origin-left transform-gpu text-white/60 transition-all duration-300 ease-in-out group-hover:scale-90 mb-2" />}
        <h3 className="text-lg font-semibold text-white">
          {name}
        </h3>
        <p className="max-w-lg text-white/40 text-sm leading-relaxed">{description}</p>
      </div>
      ...
      >
        <Button
          ...
          asChild
          size="sm"
          className="pointer-events-auto p-0 text-white/60 hover:text-white"
        >
          <a href={href}>
            {cta}
